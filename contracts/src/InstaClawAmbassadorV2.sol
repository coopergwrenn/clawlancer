...
/**
 * @title InstaClaw Ambassador V2 — Soulbound ERC-721
 ...
 *         V2: Per-token image URI (appends padded number to base image URI).
 */
contract InstaClawAmbassadorV2 is ERC721, Ownable {
     ...
     * @param newBaseImageURI The new base URI (padded ambassador number will be appended).
     */
     ...
     *         V2: Image URI = baseImageURI + paddedNum (e.g. ".../badge/001")
     */
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // V2: Append paddedNum to base image URI for per-token images
        string memory imageURI = bytes(_baseImageURI).length > 0
            ? string(abi.encodePacked(_baseImageURI, paddedNum))
            : "";
