  ...
  try {
    ...
    for (const row of rows) {
      ...
      const body = [
        ...
        "         reconcile_consecutive_failures, reconcile_quarantined_at",
        "  FROM instaclaw_vms",
        ...
        "",
        "Note: a VM here may ALSO be reconcile-quarantined (reconcile_quarantined_at",
        "set, K=10 consecutive failures, awaiting operator ack). That double-signal",
        "is EXPECTED, not a bug — the two systems detect the same stuck VM",
        "independently. A quarantined VM is excluded from reconcile-fleet and will",
        "not advance until you clear reconcile_quarantined_at after fixing the root",
        "cause (see reconcile_last_error). It correctly counts as fleet-actionable",
        "because it needs operator action to reach the manifest version.",
        "",
