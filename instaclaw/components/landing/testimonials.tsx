...
// Discord invite + member count helpers ───────────────────────────
// The invite code is the production link (matches lib/email.ts:960
// outbound signature). The public invite API needs no auth token.
const DISCORD_INVITE_CODE = "EymJzdFK9t";

function formatMemberCount(n: number): string {
  if (n < 1000) return `${n} members`;
  const k = n / 1000;
  const rounded = Math.round(k * 10) / 10; // one decimal place
  // Drop trailing .0 — 2.0 -> "2k", 1.2 -> "1.2k"
  if (rounded === Math.floor(rounded)) {
    return `${Math.floor(rounded)}k members`;
  }
  return `${rounded}k members`;
}
...
export function Testimonials() {
  // Live Discord member count via the public invite API (no auth token).
  // null while loading or on any error — pill silently falls back to just
  // "Discord" with no count rather than showing stale placeholder text.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Network error, CORS rejection, parse failure, revoked invite,
        // etc. — swallow it. Pill stays in its loading state forever
        // and renders "Discord" alone, which is the desired fallback.
      }
    })();
    return () => {
      cancelled = true;
