...
**Refinement now live (2026-06-04):** the earned budget is the keystone *governor*, but it isn't the only constraint. The number the agent (and the dashboard) reports as "what I can spend on my own right now" is the **binding minimum of three**: earned-remaining (the governor), the no-ask daily-band remaining (the tier guardrail), and wallet-headroom above the drain floor (the funding reality) — zeroed entirely when the owner hasn't opted in or the on-chain balance is unreadable. The surface names *which* factor binds (`earned` / `daily_limit` / `wallet` / `spend_disabled` / `balance_unknown`), so the agent never overstates its autonomy. Earned budget stays the thing that *grows with trust*; the other two can only ever tighten it. Lives in `lib/frontier-headroom.ts` (`autonomousHeadroom`), gate-consistency-tested 25/25 — see §2 C28 + §5.1 GAP-1.
...
| C21 | Spend kill switch — instant fleet-wide halt of autonomous spend, no deploy (DB flag in `instaclaw_admin_settings`). **FAIL-CLOSED** (Tier-0 F, 2026-06-11): an unreadable switch denies too (`spend_kill_switch_unverifiable`) — a transient blip can never bypass an engaged brake. Shared `lib/kill-switch-core.ts` (one retry → unverifiable → deny). | `lib/frontier-kill-switch.ts` (`frontierSpendKillState`) + `lib/kill-switch-core.ts` + `authorize` route | **BUILT — fail-closed; WHERE-clause prod-proven (read-only). UNPROVEN under real fire** — engage→deny→release with real spend traffic = drill D1, not yet run |
...
| C29 | **Dashboard standing read** — loads a VM's live credit standing + reserve-aware spentToday, mirroring the authorize gate's input pipeline (same pure fns) | `lib/frontier-standing-db.ts` (`loadVmStanding`) | **BUILT — live (prod, `4c47627f`)** *(consolidation done 2026-06-04: `authorize` + `/policy` GET BOTH call this — the gate's inline copy is gone; ledger-read error throws `LedgerReadError` so the gate fails CLOSED (500), `/policy` GET degrades to `autonomyError`; covered by `_test-frontier-standing-db.ts`)* |
...
### §2.1 — HARDENING LAYER (red-team F1–F11 + observability) — added 2026-06-11/12 (Rule 72)

These were built *outside* the W-item registry — the pre-announce defensibility pass that
makes Phase 1 spend agency safe to demonstrate before W12 fleet rollout. They are NOT new
phases; they are the brakes, the audit trail, and the runbook for the rail that C1–C34 built.

| ID | Item | File(s) | Status | Red-team / proof |
|----|------|---------|--------|------------------|
| H1 | **Verdict log** — one immutable `frontier_spend_events` row per authorize/settle/refund decision (budget snapshot + gate-from-reason); best-effort via `next/server` `after()` (Rule 77 sanctioned fail-open). The observability the announce ships on. | `lib/frontier-spend-log.ts` + `frontier_spend_events` table (`migrations/20260611190000`) + authorize/settle/refund routes | **BUILT — live, PROVEN RECEIVING** (real authorize on vm-050 → row SELECT'd back, prod 2026-06-11). Multi-gate real coverage = drill cross-cut (D1/D2). | A (Tier-0) |
| H2 | **Kill switches FAIL-CLOSED** — see C21; closes red-team F6 (the emergency brake was fail-OPEN). Same shared core hardened `isTravalaBookingKilled` (the literal announce verb). | `lib/kill-switch-core.ts` + frontier/travala kill-switch libs | **BUILT — closed-but-unproven-under-fire** (drill D1) | **F6 closed** |
| H3 | **Revoke interdiction** — revoke now flips in-flight `pending` holds → new `'revoked'` terminal status (settle CAS loses for free); one `revoked_in_flight` event per hold; `settle_on_revoked_hold` gap signal for revoked-but-paid. Closes red-team F3 (revoke was detection-only, not real-time interdiction). | `lib/frontier-revoke.ts` + revoke/settle routes + `'revoked'` enum (`migrations/20260611200000`) | **BUILT — closed-but-unproven-with-real-value** (drill D2) | **F3 closed** |
| H4 | **Armed-population coverage + revoke gap detector** — the blast-radius number (`frontier_spend_enabled=true`), armed ⊆ assigned invariant, revoked-but-on-chain-paid reconciliation gap. | `scripts/_coverage-frontier.ts` (Rule 27) | **BUILT — live** (armed=12, 0 leak, 2026-06-11) | B (Tier-0) |
| H5 | **Spend IR runbook + `[P1] BLIND` alert** — 10 scenarios, tested detection queries, both kill-switch engage/release SQL verbatim; the unverifiable-denial admin alert (fleet-key, 1h dedup, fails-open). | `docs/runbooks/frontier-spend-ir.md` + `authorize` route alert | **BUILT — live** | H (Tier-0) |

**Red-team ledger after the hardening pass** (`docs/frontier-redteam-2026-06-10.md`): **F1** (forgeable approval — Model-C session rail) **CLOSED+verified**; **F2** (travel consent forgeable) **CLOSED+live** (unforgeable by construction *today*, independent of the phase-3 flip); **F3** (revoke real-time) **CLOSED (H3), unproven-under-fire**; **F4** (billing/lifecycle-blind SoT) **CLOSED**; **F5** (velocity alarm) **CLOSED**; **F6** (kill-switch fail-open) **CLOSED (H2), unproven-under-fire**; **F9** (buyer-side unwind) **DESCOPED** (travala refunds = travel credits, not USDC); **F10** (per-tx scope attestation) **OPEN** — strategic for dispute-scale, explicitly *not* a canary blocker; **F11** (limits hygiene) documented, low-risk. **Only F10 remains open post-tonight.**

### §2.2 — E2E PROOF LEDGER (LIVING — rig-proven vs money-proven) — added 2026-06-12 (Rule 72)

> **Why this is a permanent PRD section, not a session note:** tonight's reconciliation proved
> session memory cannot be trusted to hold the rig-proven-vs-money-proven distinction — "done"
> silently collapses "the code is built and unit-tested" into "the money path has been
> exercised with real value," and on a money rail those are different facts. This table tracks
> the second one explicitly. **A row is `MONEY-PROVEN` only when a real-value (or real-prod-row)
> execution is cited.** Update it as drills run; never let a feature-shipped commit flip a row
> to money-proven without the cited execution.

| Money-path behavior | Rig/unit-proven | Money/prod-proven | Evidence | Gate or watch |
|---|---|---|---|---|
| allow→pay→settle (real spend) | ✓ | ✓ | on-chain `0x530cab7e` + others | proven |
| earn (agent gets paid) | ✓ | ✓ | earn canary `0xd879…` | proven |
| travel session-required unforgeable (F1/F2) | ✓ | **pending** | the travala canary booking (only completes via the session tap) | **launch gate** |
| travala book→cancel→refund | ✓ | **pending** | travala canary (vm-1043) | **launch gate** |
| **kill-switch engage→deny→release under real traffic (F6/H2)** | ✓ | **pending** | drill **D1** (this sitting) | watch → drill |
| **revoke interdicts a real in-flight hold + settle loses (F3/H3)** | ✓ | **pending** | drill **D2** (this sitting) | watch → drill |
| verdict log receives real rows across gates (H1) | ✓ | partial (1 gate: opt_in) | write-proof 2026-06-11; D1+D2 add kill_switch+revoke; booking adds session_approval | watch → drill cross-cut |
| settle_on_revoked_hold gap w/ real tx_hash | ✓ | **pending** | drill **D2b** (optional, leaves a reconcile artifact) | watch |
| spend-anomaly detection trips on real abnormal spend (F5) | ✓ | **NOT cheaply drillable BY DESIGN** | thresholds ($25 floor/$75 burst/5-distinct-cp) un-trippable by a well-behaved canary; plumbing proven via `?dryRun=true` | **permanent watch** (see drill D3 note) |
| `[P1] BLIND` unverifiable alert (H5) | ✓ | **NOT drillable** | firing it requires real Supabase sickness; we will not sabotage prod | **permanent watch** (by nature) |
| refund execution | — | **NEVER (no executor exists)** | runbook S6 = manual USDC send | **structural gap** (prior-art #1) |
| W12 fleet spend (≥50 VMs) | n/a | **NEVER** | the rollout itself | **the big gate** |
...
| **Phase 2 — Fleet + feed** 🟡 PARTIAL | W9, W12, W13, W14 | Roll spend to the fleet (Rule 64) + ship the viral dashboard feed; W10 graph starts accruing real data | **Dashboard money-surface live in prod** (`/economy` page + per-band tightening control + earned-autonomy snapshot — §5.1 GAP-1/GAP-2, §2 C18/C28–C34). **W12 fleet rollout** gated on Phase 1 green + Rule 64 + the two Cooper calls; **W13/W14 transaction feed NOT built** (data ready in `/state` `recent[]`); W9 not started. |
...
**Status (updated 2026-06-12):** Phase 0 = done. **Phase 1 = code-complete and canary-proven** (W1–W8, W11 all built; W11 harness passes 6/6 live; see §6 + §2 registry C2–C10, C21–C26). **Phase 1 is now also HARDENED** — the red-team F1–F11 pass + the Tier-0 observability layer (verdict log, fail-closed kill switches, revoke interdiction, IR runbook) are **built and live** (see new §2.1; only F10 open). **BUT — see §2.2: every brake in the hardening layer is RIG-PROVEN, not yet MONEY-PROVEN.** F6/H2 (kill switch) and F3/H3 (revoke) have never been pulled with real value in motion — that's the brake-drills sitting (`docs/runbooks/frontier-canary-sitting-plan.md`), which runs in the same session as the travala canary. **Phase 2 = partially shipped:** the dashboard money-surface (`/economy` page + per-band tightening + earned-autonomy snapshot — §5.1 GAP-1/GAP-2, §2 C18/C28–C34) is **live in prod**, but **W12 (fleet rollout, the load-bearing Phase-2 step) is NOT done** — it awaits **Cooper greenlight + the two §5 calls (Q1 funding, Q3 no-ask line)** — and **W13 (transaction feed) is NOT built** (data ready in `/state` `recent[]`); **W9/W10 not started**. The new spend skill is canary-only until W12's manifest version bump carries it fleet-wide. Phases 3–4 and the §9 card rail remain future/not-started.
...
7. **Dashboard ownership. ◑ OPEN — Cooper's coordination call.** The Agent Economy dashboard surface (W13/W14) was built across more than one terminal, not owned by one: the frontier terminal shipped the `/economy` money-surface (the spend-policy controls + the earned-autonomy snapshot — §2 C18/C28–C34) on 2026-06-03→04, alongside adjacent dashboard work owned by other terminals. No single terminal owns the surface by default. Recommendation: keep the economy money-surface with the frontier terminal for continuity; formal assignment of the dashboard surface as a whole remains Cooper's call.
...
11. **⏳ OPEN — the deepest unsolved problem in this PRD (logged verbatim 2026-06-12, prior-art #3; must not get lost):** **How does earned autonomy resist an agent gaming its own track record?** The budget grows on "settled ∧ used ∧ undisputed ∧ not-self-dealt" (§7.3.2) — but on a non-custodial rail, **the settle outcome is the agent's own word.** `tx_hash` proves money *moved*, never that the purchase was *useful*. An agent can climb the autonomy ladder by reporting every spend as a success: `result.used=true`, no dispute. The §7.3.1 anti-wash-trade weighting (same-human exclusion, counterparty diversity, cost-anchoring) stops *sybil* (faking counterparties), but not an agent *lying to itself* about its own outcomes. The only corrective is the dispute model — which §10.4 risk #1 / Q20 already names as **"the single biggest product risk … a real gap."** This is a *governance* problem, not a feature, which is exactly why it's easy to ship around and dangerous to leave. **There are no incumbents to copy here — Stripe/the OTAs never built earned-autonomy-from-self-reported-outcomes; whatever we choose becomes the prior art.** Candidate directions (not decided): (a) on-chain settlement verification so "settled" can't be claimed without a real tx (partially exists — `frontier-chain-verify`); (b) outcome-attestation by the *counterparty* (the supplier confirms delivery), not the buyer; (c) decay/discount of self-reported "used" until a dispute window passes; (d) cap earned-autonomy growth rate so gaming is slow + detectable. **Needs a Cooper + first-principles answer before earned-autonomy is the headline of a public announce or before W12 widens the gameable surface to the fleet.**
...
- **Hardening (this session):** spend kill switch (C21), write-once latency+failure capture (C22), atomic reserve RPC / P1-4 (C23), spend-health alerting cron / P2-6 (C24), standing-truncation flag / P2-8 (C26). Frontier test suite: **544 passed / 0 failed across the 15 `_test-frontier-*.ts` suites.** (GAP-4: `_test-frontier-policy-route.ts` covers the money-gate's override reader + PUT validator + the read→effective→evaluateSpend wiring. C29 net: `_test-frontier-standing-db.ts` covers `loadVmStanding`'s fetch contract + fail-closed `LedgerReadError`.)
