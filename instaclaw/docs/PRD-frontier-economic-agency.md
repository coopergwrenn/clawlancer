...
- **Hardening (this session):** spend kill switch (C21), write-once latency+failure capture (C22), atomic reserve RPC / P1-4 (C23), spend-health alerting cron / P2-6 (C24), standing-truncation flag / P2-8 (C26). Frontier test suite: **527 passed / 0 failed across the 14 `_test-frontier-*.ts` suites.** (GAP-4: `_test-frontier-policy-route.ts` covers the money-gate's override reader + PUT validator + the read→effective→evaluateSpend wiring.)
