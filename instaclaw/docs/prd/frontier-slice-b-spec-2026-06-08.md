...
- **#5b — anomaly ask.**
  - **(a) Code touched:** route passes `standing`'s underlying `anomalyFlag` (already computed in `deriveTrackRecord:212-214`, already lowers the score via `computeFactors:85`) as a *direct* gate input; gate adds `if (anomalyFlag) → ask_first`. Edits `decideAuthorization`.
