...
2. **Disable the old Muapi `higgsfield-video` skill on vm-050** (rename `.disabled`) so the agent routes to `higgsfield-cloud`, not the old rail (else the canary tests the wrong path). **(M6)**
