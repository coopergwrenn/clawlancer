...
# ════════════════════════════════════════════════════════════════════════
# §1.0.7 BEST_EFFORT: disable OpenClaw bonjour (mDNS) on cloud VMs
# ════════════════════════════════════════════════════════════════════════
# Bonjour is OpenClaw's local-network discovery service that announces the
# gateway via mDNS. On a Linode VM with no LAN peers, this is pure waste —
# and it actively HURTS startup time. The bonjour announcer can stall in
# "stuck announcing" state for 60-90 seconds on cold Linodes (verified on
# vm-973/974/975), blocking the gateway's [gateway] ready event for that
# duration. The §1.32 health probe (240s budget per fix #6) absorbs this,
# but the cost is real wall-clock UX delay between callback and bot-ready.
#
# OpenClaw 2026.4.26 has a documented disable mechanism (verified by
# reading dist/server.impl-hNr66nDN.js):
#
#   function startGatewayDiscovery(params) {
#     const mdnsMode = params.mdnsMode ?? "minimal";
#     const localDiscoveryEnabled = mdnsMode !== "off"
#       && !isTruthyEnvValue(process.env.OPENCLAW_DISABLE_BONJOUR)
#       && !process.env.VITEST;
#     ...
#   }
#
# Two gates: discovery.mdns.mode = "off" in openclaw.json (already set by
# buildOpenClawConfig) AND env var OPENCLAW_DISABLE_BONJOUR=true. Either
# disables the announcer. The env var path is more robust because it
# bypasses config-load-time issues if openclaw.json ever drifts.
#
# Safety analysis (per Cooper's directive — only ship if genuinely safe):
#   - Bonjour serves agent-to-agent discovery on a LAN. Cloud VMs have no
#     LAN peers (each Linode is in its own network segment). Disabling it
#     removes a feature that produces zero value on this VM class.
#   - Gateway HTTP server, Telegram polling, /api/gateway proxy, heartbeat,
#     browser plugin — none use bonjour. Verified by grep across the entire
#     OpenClaw dist (only bonjour-discovery-*.js + gateway-discovery-*.js
#     reference bonjour at all, and those are the bonjour subsystem itself).
#   - Pool-path canary (vm-050 edge_city, 2026-05-22 00:22 UTC): added the
#     same drop-in, restarted, gateway came up cleanly (active, no warnings,
#     telegram polling resumed). Bonjour-disable IS safe in practice.
#   - Win on vm-050: marginal (97s → 94s — bonjour wasn't the dominant
#     delay there; acpx took 78s). On cloud-init VMs (vm-975): bonjour WAS
#     the dominant 85s delay. So the win is variable per VM but never
#     negative.
#
# Approach: systemd drop-in at the standard user-unit path. Idempotent
# (cat > overwrite is fine since the content is canonical). daemon-reload
# happens automatically when systemctl --user restart runs (later in §1.32)
# OR on next gateway boot (which is imminent after this step).
#
# Future: same drop-in could go in the snapshot bake recipe (covers pool
# VMs too). For NOW, setup.sh covers the cloud-init code path that's the
# Edge Esmeralda priority. Tracked as snapshot-bake followup.
{
  install -d -o openclaw -g openclaw -m 755 /home/openclaw/.config/systemd/user/openclaw-gateway.service.d
  cat > /home/openclaw/.config/systemd/user/openclaw-gateway.service.d/99-disable-bonjour.conf <<'BONJOUR_DROPIN_EOF'
# Generated by cloud-init setup.sh §1.0.7 — DO NOT EDIT MANUALLY
# Disables OpenClaw's bonjour mDNS announcer on cloud VMs (no LAN peers
# means bonjour is pure waste + can stall gateway startup 60-90s).
# Read by OpenClaw 2026.4.26's startGatewayDiscovery (dist/server.impl).
[Service]
