/**
 * Skill icon system:
 * - Brand logos: simple-icons SVG paths (Solana, X, Google, Notion, Shopify, GitHub, Apple, Trello)
 * - Brand logos (no simple-icon): image files (Higgsfield, Virtuals, Clawlancer)
 * - Generic skills: Lucide React components
 */
...
const IMAGE_ICONS: Record<string, string> = {
  higgsfield: "/skill-icons/higgsfield.jpg",
  virtuals: "/skill-icons/virtuals.png",
  clawlancer: "/skill-icons/clawlancer.png",
};

// ── Lucide icon mapping for generic skills ──────────
...
function resolveIcon(slug: string): IconSource {
  // Check brand SVG icons
  if (slug === "solana-defi") return { type: "brand", key: "solana" };
  if (slug === "x-twitter-search") return { type: "brand", key: "x" };
  if (slug === "google-workspace") return { type: "brand", key: "google" };
  if (slug === "notion") return { type: "brand", key: "notion" };
  if (slug === "shopify") return { type: "brand", key: "shopify" };
  if (slug === "github") return { type: "brand", key: "github" };
  if (slug === "apple-notes" || slug === "apple-reminders")
    return { type: "brand", key: "apple" };
  if (slug === "trello") return { type: "brand", key: "trello" };
  if (slug === "slack") return { type: "lucide", Icon: MessageSquare };

  // Check image-based brand logos
  if (slug === "higgsfield-video")
    return { type: "image", src: IMAGE_ICONS.higgsfield };
  if (slug === "virtuals-agdp")
    return { type: "image", src: IMAGE_ICONS.virtuals };
  if (slug === "clawlancer")
    return { type: "image", src: IMAGE_ICONS.clawlancer };

  // Check Lucide icons
  const lucide = LUCIDE_ICONS[slug];
  if (lucide) return { type: "lucide", Icon: lucide };
...
export function SkillIcon({
  ...
  const source = resolveIcon(slug);
  if (!source) return null;

  if (source.type === "brand") {
    const brand = BRAND_ICONS[source.key];
    if (!brand) return null;
    const gradientId = `grad-${source.key}`;
    return (
      <svg
        width={size}
        height={size}
        viewBox="0 0 24 24"
        fill={brand.gradient ? `url(#${gradientId})` : brand.hex}
        className={className}
        role="img"
      >
        {brand.gradient && (
...
/** Check if a slug has a custom icon (brand, image, or lucide) */
