...
/**
 * BROWSER_USE_TASK_SCRIPT — CLI wrapper for the browser-use Python package
 * (InstaClaw Tier 3.25 in the web-search-browser ladder).
 *
 * Deployed to ~/scripts/browser-use-task.py via VM_MANIFEST.files (templateKey
 * "BROWSER_USE_TASK_SCRIPT"). Runtime depends on the `browser-use` pip package,
 * which is installed by the parallel pip block in configureOpenClaw().
 *
 * KEEP IN SYNC with instaclaw/skills/browser-use/assets/browser-use-task.py.
 * The .py file is the canonical version (used for Phase 0 canary testing);
 * this TS constant is the deploy-time copy. To verify they match:
 *   diff <(node -e "console.log(require('./instaclaw/lib/ssh').BROWSER_USE_TASK_SCRIPT)") instaclaw/skills/browser-use/assets/browser-use-task.py
 *
 * The .py source has no backticks, no backslashes, no \${...} patterns —
 * verified before inlining (PRD §3.2 wrapper spec). Pasting verbatim is safe.
 */
export const BROWSER_USE_TASK_SCRIPT = `#!/usr/bin/env python3
...
registerTemplate("BROWSER_USE_TASK_SCRIPT", BROWSER_USE_TASK_SCRIPT);
...
`;
    ...
    // All 6 independent package installs run concurrently with & + wait.
    ...
    scriptParts.push(
      ...
      'PID_SOLANA=$!',
      '',
      '# 5. browser-use pip install (browser-use skill — Tier 3.25)',
      '#    Pip name (browser-use) ≠ import name (browser_use), so this is NOT in pythonPackages.',
      '#    Same precedent as Crawlee — pinned conservatively against minor-release breakage.',
      ...
      '',
      '# Verify browser-use installed (Tier 3.25 — non-blocking, just signals failure to logs)',
