...
export async function configureOpenClaw(
  ...
  try {
    ...
    scriptParts.push(
      ...
      '',
      '# v118 (2026-05-23) — typing-keepalive patch for telegram bot.',
      '#',
      '# Patches ~/.nvm/.../openclaw/dist/extensions/telegram/bot-msflwCEW.js',
      '# to refresh sendChatAction("typing") every 4s during a turn (Telegram',
      '# typing TTL is 5s; without keepalive, the indicator dies during any',
      '# LLM call > 5s, leaving dead-typing-air during which status reactions',
      '# look like "weird stuff happening on my message" instead of progress).',
      '#',
      '# Patch is idempotent (sentinel-grep skips if already present),',
      '# atomic (verify node --check before swap), and re-applied on every',
      '# configure (openclaw reinstalls wipe this — see CLAUDE.md Rule 63).',
      '#',
      '# Source: 2026-05-23 vm-1019 Cooper choppy-UX investigation.',
      "{ set -u; TARGET=\"$HOME/.nvm/versions/node/v22.22.2/lib/node_modules/openclaw/dist/extensions/telegram/bot-msflwCEW.js\"",
      '  SENTINEL="INSTACLAW v118 typing-keepalive shim"',
      '  if [ ! -f "$TARGET" ]; then',
      '    echo "TK_PATCH: target missing, skipping non-fatally"',
      '  elif grep -qF "$SENTINEL" "$TARGET"; then',
      '    echo "TK_PATCH: already patched (sentinel found)"',
      '  else',
      '    cp -p "$TARGET" "$TARGET.pre-typing-keepalive-bak"',
      "    python3 - << 'TKPYEOF'",
      'import os, sys',
      'TARGET = os.path.expanduser("~/.nvm/versions/node/v22.22.2/lib/node_modules/openclaw/dist/extensions/telegram/bot-msflwCEW.js")',
      'SENTINEL = "INSTACLAW v118 typing-keepalive shim"',
      'with open(TARGET) as f: src = f.read()',
      'if SENTINEL in src:',
      '    print("TK_PATCH: race — sentinel already present, aborting")',
      '    sys.exit(0)',
      'ORIG_ST = (',
      ...
      ')',
      'NEW_ST = (',
      '    \'\\t// \' + SENTINEL + \' — DO NOT EDIT (re-applied by configureOpenClaw)\\n\'',
      ...
      '    \'\\t\\t\\tfn: () => sendChatActionHandler.sendChatAction(chatId, "typing", buildTypingThreadParams(replyThreadId))\\n\'',
      '    \'\\t\\t});\\n\'',
      '    \'\\t\\t__instaclawTKStart(() => sendChatActionHandler.sendChatAction(chatId, "typing", buildTypingThreadParams(replyThreadId)));\\n\'',
      '    \'\\t};\'',
      ')',
      'if ORIG_ST not in src or src.count(ORIG_ST) != 1:',
      '    print(f"TK_PATCH: FAIL — sendTyping anchor count={src.count(ORIG_ST)} (expected 1)")',
      '    sys.exit(2)',
      'src = src.replace(ORIG_ST, NEW_ST, 1)',
      'ORIG_F = \'if (info.kind === "final") await enqueueDraftLaneEvent(async () => {});\'',
      'NEW_F = \'if (info.kind === "final") { try { __instaclawTKStop(); } catch (_) {} await enqueueDraftLaneEvent(async () => {}); }\'',
      'if ORIG_F not in src or src.count(ORIG_F) != 1:',
      '    print(f"TK_PATCH: FAIL — deliver-final anchor count={src.count(ORIG_F)} (expected 1)")',
      '    sys.exit(2)',
      'src = src.replace(ORIG_F, NEW_F, 1)',
      'with open(TARGET + ".tmp.js", "w") as f: f.write(src)',
      'print(f"TK_PATCH: wrote {len(src)} chars to .tmp.js for syntax check")',
      'TKPYEOF',
      '    PY_RC=$?',
      '    if [ $PY_RC -ne 0 ] || [ ! -f "$TARGET.tmp.js" ]; then',
      '      echo "TK_PATCH: python patcher failed (rc=$PY_RC); leaving original intact"',
      '      rm -f "$TARGET.tmp.js" 2>/dev/null',
      '    else',
      '      NODE_BIN=$(command -v node || ls -1 "$HOME"/.nvm/versions/node/*/bin/node 2>/dev/null | head -1)',
      '      if [ -n "$NODE_BIN" ] && "$NODE_BIN" --check "$TARGET.tmp.js" >/dev/null 2>&1; then',
      '        mv "$TARGET.tmp.js" "$TARGET"',
      '        echo "TK_PATCH: applied successfully (backup at $TARGET.pre-typing-keepalive-bak)"',
      '      else',
      '        echo "TK_PATCH: node --check FAILED on patched file; leaving original intact"',
      '        rm -f "$TARGET.tmp.js"',
      '      fi',
      '    fi',
      '  fi',
      '} || echo "TK_PATCH: outer block failed non-fatally"',
      '',
