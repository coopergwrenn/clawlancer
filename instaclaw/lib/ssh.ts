...
export async function restartGateway(vm: VMRecord): Promise<boolean> {
  ...
  try {
    const script = [
      ...
      'cp ~/.openclaw/openclaw.json /tmp/openclaw-backup.json 2>/dev/null || true',
      '',
      '# Fix 6: Validate openclaw.json before restart — prevent crash loops',
      '# If controlUi is set but allowedOrigins is missing, add safe default',
      'python3 -c \'',
      'import json, sys',
      'try:',
      ...
      '  if cui and "dangerouslyAllowHostHeaderOriginFallback" not in cui and "allowedOrigins" not in cui:',
      ...
      '    print("CONFIG_FIXED: added controlUi.dangerouslyAllowHostHeaderOriginFallback")',
      '  else:',
      '    print("CONFIG_OK")',
