...
export const VM_MANIFEST = {
  ...
  files: [
    ...
    // ─── Bonjour mDNS disable drop-in (Rule 62 P1 followup, 2026-05-28) ───
    //
    // Prior to this entry, ~/.config/systemd/user/openclaw-gateway.service.d/
    // 99-disable-bonjour.conf was only written by configureOpenClaw at user
    // assignment AND by cloud-init's setup.sh §1.0.7 at first boot. Fresh
    // pool VMs provisioned from the snapshot had a window between snapshot-
    // boot and configureOpenClaw where bonjour was NOT disabled — the bonjour
    // PLUGIN ADVERTISER blocks the Node event loop trying to advertise
    // localhost.local (no LAN peers on a cloud VM) and only self-disables
    // after 3 failed restarts (~4-7 min). vm-1019's 2026-05-23 6-min first-
    // message latency was the smoking gun (Cooper).
    //
    // Adding this entry makes stepFiles write the drop-in during reconcile,
    // so the bake VM captures it into the snapshot. Fresh provisions ship
    // with bonjour disabled from snapshot-boot.
    //
    // create_if_missing mode (NOT overwrite): coexists peacefully with
    // configureOpenClaw's writer (lib/ssh.ts:8633) and cloud-init's writer
    // (lib/cloud-init-setup-sh.ts §1.0.7). Both write functionally-equivalent
    // content with slightly different comment headers. file-drift cron sees
    // file exists, no-op. No drift war.
    //
    // CLAUDE.md Rule 62 dual-gate: this drop-in covers the env var ADVERTISER
    // gate; discovery.mdns.mode=off in openclaw.json (set by configureOpenClaw)
    // covers the DISCOVERY client gate. Both required — see Rule 62 body.
    {
      remotePath: "~/.config/systemd/user/openclaw-gateway.service.d/99-disable-bonjour.conf",
      source: "inline",
      content: [
        "# Generated by InstaClaw platform (vm-manifest.ts:files[]) — DO NOT EDIT MANUALLY",
        "# Disables OpenClaw's bonjour mDNS advertiser on cloud VMs (CLAUDE.md Rule 62).",
        "# Dual-gate: discovery.mdns.mode=off (config) + OPENCLAW_DISABLE_BONJOUR=true (this).",
        "[Service]",
