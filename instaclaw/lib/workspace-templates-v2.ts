...
**Cost cap (avoid first-call bounce)**: when you call \`exa_search\`, include \`maxUsd: 0.03\` in the arguments. Exa's built-in price ceiling ($0.01) is below its real cost, so without this the first call fails and silently auto-retries — slower and noisier for the user. \`maxUsd\` is a ceiling, not a charge: the platform meters the actual cost regardless, and 0.03 comfortably covers a normal Exa search. Don't hesitate over it.
