#!/bin/bash
#
# fleet-push-marketplace-skill.sh — Deploy marketplace earning & digital product skill to existing VMs
#
# Pushes: SKILL.md (doc-only skill, no references directory)
# No external API keys needed — uses Clawlancer bounties + browser automation (pre-installed).
#
# Usage:
#   fleet-push-marketplace-skill.sh --dry-run    — Preview deployment
#   fleet-push-marketplace-skill.sh --canary     — Deploy to 1 VM, pause for approval
#   fleet-push-marketplace-skill.sh --all        — Deploy to all active VMs
#
...
SKILL_DIR="$PROJECT_ROOT/skills/marketplace-earning"
...
SKILL_DIR="\$HOME/.openclaw/skills/marketplace-earning"
mkdir -p "\$SKILL_DIR"

echo '$skill_md_b64' | base64 -d > "\$SKILL_DIR/SKILL.md"

echo "  Marketplace earning skill deployed successfully"
REMOTE_SCRIPT
...
case "$MODE" in
  --dry-run)
    echo "=== DRY RUN: Marketplace Earning & Digital Product Skill Deployment ==="
    echo ""
    echo "Files to deploy:"
    echo "  SKILL.md  -> ~/.openclaw/skills/marketplace-earning/SKILL.md"
    echo ""
    echo "No API keys required — uses Clawlancer bounties + browser automation (pre-installed)."
    echo ""
...
" 2>/dev/null)
    ...
    echo "Verify: ssh ${USER}@${IP} 'ls ~/.openclaw/skills/marketplace-earning/'"
    echo "If healthy, run: $0 --all"
    ;;

  --all)
    echo "=== FLEET DEPLOY: Marketplace Earning & Digital Product Skill ==="
    VMS=$(fetch_vms)
...
" 2>/dev/null | while read -r IP USER VM_ID; do
  ...
  --help|*)
    echo "fleet-push-marketplace-skill.sh — Deploy marketplace earning skill to fleet"
    echo ""
    echo "Usage:"
