#!/bin/bash
#
# fleet-push-web-skill.sh — Deploy web search & browser automation skill to existing VMs
#
# Pushes: SKILL.md, references/browser-patterns.md
# No external API keys needed — all tools are built-in on the VM.
#
# Usage:
#   fleet-push-web-skill.sh --dry-run    — Preview deployment
#   fleet-push-web-skill.sh --canary     — Deploy to 1 VM, pause for approval
#   fleet-push-web-skill.sh --all        — Deploy to all active VMs
#
...
SKILL_DIR="$PROJECT_ROOT/skills/web-search-browser"
...
deploy_to_vm() {
  ...
  patterns_b64=$(base64 < "$SKILL_DIR/references/browser-patterns.md")

  ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o BatchMode=yes -i "$SSH_KEY_FILE" "${user}@${ip}" bash -s <<REMOTE_SCRIPT
set -e

SKILL_DIR="\$HOME/.openclaw/skills/web-search-browser"
mkdir -p "\$SKILL_DIR/references"

echo '$skill_md_b64' | base64 -d > "\$SKILL_DIR/SKILL.md"
echo '$patterns_b64' | base64 -d > "\$SKILL_DIR/references/browser-patterns.md"

echo "  Web search & browser automation skill deployed successfully"
REMOTE_SCRIPT
...
case "$MODE" in
  --dry-run)
    echo "=== DRY RUN: Web Search & Browser Automation Skill Deployment ==="
    echo ""
    echo "Files to deploy:"
    echo "  SKILL.md               -> ~/.openclaw/skills/web-search-browser/SKILL.md"
    echo "  browser-patterns.md    -> ~/.openclaw/skills/web-search-browser/references/"
    echo ""
    echo "No API keys required — all tools are built-in on the VM."
    echo ""
...
" 2>/dev/null)
    ...
    echo "Verify: ssh ${USER}@${IP} 'ls ~/.openclaw/skills/web-search-browser/'"
    echo "If healthy, run: $0 --all"
    ;;

  --all)
    echo "=== FLEET DEPLOY: Web Search & Browser Automation Skill ==="
    VMS=$(fetch_vms)
...
" 2>/dev/null | while read -r IP USER VM_ID; do
  ...
  --help|*)
    echo "fleet-push-web-skill.sh — Deploy web search & browser automation skill to fleet"
    echo ""
    echo "Usage:"
