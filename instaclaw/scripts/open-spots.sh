      ...
      for attempt in $(seq 1 60); do
        ...
        if [ "$RESULT" = "READY" ]; then
          ...
          curl -s -X PATCH "${SUPABASE_URL}/rest/v1/instaclaw_vms?ip_address=eq.${VM_IP}" \
            ...
            -H "Content-Type: application/json" \
            -d '{"status":"ready"}' > /dev/null
          # Set AGENT_REGION and POLYGON_RPC_URL in ~/.openclaw/.env
          echo "    Setting AGENT_REGION=${LINODE_REGION} and POLYGON_RPC_URL..."
          ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o BatchMode=yes \
            -i "$SSH_KEY_FILE" "openclaw@${VM_IP}" \
            "mkdir -p ~/.openclaw && grep -q AGENT_REGION ~/.openclaw/.env 2>/dev/null || echo 'AGENT_REGION=${LINODE_REGION}' >> ~/.openclaw/.env && grep -q POLYGON_RPC_URL ~/.openclaw/.env 2>/dev/null || echo 'POLYGON_RPC_URL=https://1rpc.io/matic' >> ~/.openclaw/.env" 2>/dev/null || true
          CLOUD_INIT_DONE=true
          READY_COUNT=$((READY_COUNT + 1))
          break
        fi
