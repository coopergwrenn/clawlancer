...
Usage:
  python3 ~/scripts/polymarket-trade.py buy --market-id <id> --outcome YES --amount 10 [--price 0.65] [--type GTC] [--json]
  python3 ~/scripts/polymarket-trade.py sell --market-id <id> --outcome YES --shares 15 [--price 0.70] [--type GTC] [--json]
...
def cmd_buy(args):
    ...
    try:
        ...
        order_type = order_type_map.get(args.type, OrderType.GTC)
...
def cmd_sell(args):
    ...
    # 10. Update positions
    update_positions(args.market_id, market_question, args.outcome, token_id, args.shares, price, "SELL")
    ...
    if args.json:
        ...
        print(f"OK — SELL order placed")
        print(f"  Market: {market_question}")
        print(f"  Outcome: {args.outcome}")
        print(f"  Shares: {args.shares:.2f}")
        print(f"  Price: ${price:.4f} ({price*100:.1f}% implied)")
...
def main():
    ...
    sp_buy.add_argument("--market-id", required=True, help="Gamma API market ID")
    ...
    sp_buy.add_argument("--price", type=float, help="Limit price (0-1). Uses market price if omitted.")
    sp_buy.add_argument("--type", default="GTC", choices=["GTC", "FOK"], help="Order type (default: GTC)")
    ...
    sp_sell.add_argument("--market-id", required=True, help="Gamma API market ID")
    ...
    sp_sell.add_argument("--price", type=float, help="Limit price (0-1). Uses market price if omitted.")
    sp_sell.add_argument("--type", default="GTC", choices=["GTC", "FOK"], help="Order type (default: GTC)")
    ...
    # orders
    sp_orders = subparsers.add_parser("orders", help="List open orders")
