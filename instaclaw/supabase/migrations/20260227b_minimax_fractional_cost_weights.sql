-- Add MiniMax M2.5 support with fractional cost weights.
--
-- MiniMax M2.5 costs 0.2 units per message (5x cheaper than Haiku).
-- This requires changing cost_weight from INTEGER to NUMERIC, and
-- updating the usage columns to NUMERIC so fractional values work.

-- 1. Change usage columns from INTEGER to NUMERIC
ALTER TABLE instaclaw_daily_usage
  ALTER COLUMN message_count TYPE NUMERIC USING message_count::NUMERIC;
ALTER TABLE instaclaw_daily_usage
  ALTER COLUMN heartbeat_count TYPE NUMERIC USING heartbeat_count::NUMERIC;
ALTER TABLE instaclaw_vms
  ALTER COLUMN credit_balance TYPE NUMERIC USING credit_balance::NUMERIC;

-- 2. Recreate check_limit_only with MiniMax support + NUMERIC cost_weight
...
-- 3. Recreate increment_usage with MiniMax support + NUMERIC cost_weight
