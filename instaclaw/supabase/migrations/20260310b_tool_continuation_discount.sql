-- Tool-use continuation credit discount (0.2x multiplier).
--
-- Problem: Every API call costs the same credits whether it's a user-initiated
-- message or a tool-use continuation within an agent turn. A 50-step Sonnet
-- tool chain costs 50 x 4 = 200 credits.
--
-- Fix: Detect tool_result messages (agent calling tools mid-turn) and charge
-- them at 0.2x the normal model weight. The same 50-step chain now costs
-- 4 + (49 x 0.8) = 43.2 credits.
--
-- Detection: The proxy inspects the last message in the request. If it has
-- role:"user" with a type:"tool_result" content block, it's a continuation.

-- 1. Recreate instaclaw_check_limit_only with p_is_tool_continuation parameter
DROP FUNCTION IF EXISTS instaclaw_check_limit_only(UUID, TEXT, TEXT, BOOLEAN, TEXT, BOOLEAN);
...
-- 2. Recreate instaclaw_increment_usage with p_is_tool_continuation parameter
DROP FUNCTION IF EXISTS instaclaw_increment_usage(UUID, TEXT, BOOLEAN, TEXT, BOOLEAN);
