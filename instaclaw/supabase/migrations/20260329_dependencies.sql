...
INSERT INTO instaclaw_dependencies (name, description, category, check_type, check_target, repo_url, our_version, update_impact, notes) VALUES
  ...
  ('Privy', 'Web3 wallet authentication for humans', 'core', 'manual', NULL, 'https://privy.io', NULL, 'high', 'Wallet auth provider. No SDK versioning — monitor dashboard for breaking changes.'),
  ('NextAuth.js', 'Authentication framework (Google OAuth + sessions)', 'core', 'manual', NULL, 'https://github.com/nextauthjs/next-auth', '5.0.0-beta.30', 'high', 'Intentionally on v5 beta track (5.0.0-beta.30). Stable v4.x is a different major version. Do not auto-compare.'),
  ('node-ssh', 'SSH client for VM provisioning and config deployment', 'core', 'npm', 'node-ssh', 'https://github.com/steelbrain/node-ssh', '13.2.1', 'high', 'Critical infrastructure. Breaking changes could break entire deployment pipeline.'),
  ...
  ('GoDaddy DNS', 'DNS A record management for VM HTTPS certs', 'api', 'manual', NULL, 'https://developer.godaddy.com', NULL, 'high', 'CRITICAL — VM HTTPS depends on this. Outage = new VMs cannot get certs.'),
  ('Telegram Bot API', 'User-agent messaging via Telegram bots', 'api', 'http_health', 'https://api.telegram.org', NULL, NULL, 'medium', 'Schema changes could break message routing. Monitor Bot API changelog.'),
  ('Alpha Vantage', 'Stock market data for financial-analysis skill', 'api', 'manual', NULL, 'https://www.alphavantage.co', NULL, 'medium', '500 calls/day free tier. Rate limit changes affect agents.'),
  ('py-clob-client', 'Python SDK for Polymarket CLOB trading', 'skill', 'pypi', 'py-clob-client', 'https://github.com/polymarket/clob-client', '0.34.6', 'high', 'Used in 6+ prediction market scripts. Unpinned on VMs — version from live fleet.'),
